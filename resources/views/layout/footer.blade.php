<!-- resources/views/partials/footer.blade.php -->
<footer class="bg-dark text-white mt-5">
    <div class="container py-4">
        <div class="row">
            <div class="col-md-3">
                <h5>VỀ CHÚNG TÔI</h5>
                <ul class="list-unstyled">
                    <li><a href="#" class="text-white text-decoration-none">About Us</a></li>
                </ul>
            </div>
            <div class="col-md-3">
                <h5>HỖ TRỢ KHÁCH HÀNG</h5>
                <ul class="list-unstyled ">
                    <li><a href="#" class="text-white text-decoration-none">Giới thiệu</a></li>
                    <li><a href="#" class="text-white text-decoration-none">Chính thức hỗ trợ</a></li>
                    <li><a href="#" class="text-white text-decoration-none">Chính thức đổi trả</a></li>
                </ul>
            </div>
            <div class="col-md-3">
                <h5>TRUY CẬP NHANH</h5>
                <ul class="list-unstyled">
                    <li><a href="#" class="text-white text-decoration-none">Mua hàng</a></li>
                    <li><a href="#" class="text-white text-decoration-none">Giỏ hàng</a></li>
                    <li><a href="#" class="text-white text-decoration-none">Đơn hàng Policy</a></li>
                </ul>
            </div>
            <div class="col-md-3">
                <h5>FANPAGE</h5>
                <ul class="list-unstyled">
                    <li><a href="#" class="text-white text-decoration-none">Facebook</a></li>
                    <li><a href="#" class="text-white text-decoration-none">Twitter</a></li>
                    <li><a href="#" class="text-white text-decoration-none">Instagram</a></li>
                </ul>
            </div>
        </div>
        <div class="text-center mt-4">
            <p class="mb-0">© 2024 Demo website bán hàng.</p>
        </div>
    </div>
</footer>
